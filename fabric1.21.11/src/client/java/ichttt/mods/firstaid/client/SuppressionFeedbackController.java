...
public final class SuppressionFeedbackController {
    private static final Identifier TINNITUS_RING_SOUND = Identifier.withDefaultNamespace("block.bell.use");
    private static final Identifier TINNITUS_LAYER_SOUND = Identifier.withDefaultNamespace("block.conduit.ambient.short");
    private static final Identifier MUFFLE_PUNCH_SOUND = Identifier.withDefaultNamespace("entity.warden.heartbeat");
    private static final Set<Identifier> INTERNAL_SOUNDS = Set.of(TINNITUS_RING_SOUND, TINNITUS_LAYER_SOUND, MUFFLE_PUNCH_SOUND);
    ...
    private void playSuppressionSounds(float severity) {
        ...
        SoundEvent ring = BuiltInRegistries.SOUND_EVENT.getValue(TINNITUS_RING_SOUND);
        if (ring != null) {
            soundManager.play(SimpleSoundInstance.forUI(ring, 0.16F + severity * 0.24F, 1.60F + severity * 0.26F));
        }
        SoundEvent layer = BuiltInRegistries.SOUND_EVENT.getValue(TINNITUS_LAYER_SOUND);
        if (layer != null) {
            soundManager.play(SimpleSoundInstance.forUI(layer, 0.12F + severity * 0.18F, 1.22F + severity * 0.18F));
        }
        SoundEvent punch = BuiltInRegistries.SOUND_EVENT.getValue(MUFFLE_PUNCH_SOUND);
        if (punch != null) {
